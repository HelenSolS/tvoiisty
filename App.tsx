...
import { describeOutfit, generateTryOn } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleQuickTryOn = async (outfitUrl: string, shopUrl: string = '#') => {
    ...
    setState(prev => ({ ...prev, outfitImage: outfitUrl, currentShopUrl: shopUrl, isProcessing: true, status: 'Анализ стиля...', error: null }));
    try {
      ...
      const result = await generateTryOn(personBase64, outfitBase64, description);
      if (!result) throw new Error("Ошибка генерации");
      setState(prev => ({ ...prev, resultImage: result, isProcessing: false, status: '' }));
      const newItem: HistoryItem = { id: `h_${Date.now()}`, resultUrl: result, outfitUrl, shopUrl, timestamp: Date.now() };
      const newHistory = [newItem, ...history].slice(0, 20);
      setHistory(newHistory);
      saveToStorage('history', newHistory);
    } catch (err: any) {
      setState(prev => ({ ...prev, isProcessing: false, error: 'ИИ перегружен, попробуйте снова' }));
  ...
  const goToTab = (tab: 'home' | 'history' | 'settings' | 'showroom') => {
    setActiveTab(tab);
    if (tab === 'home') setState(s => ({ ...s, resultImage: null, error: null }));
  };
        ...
        {state.resultImage ? (
          <div className="p-7 space-y-8 animate-in slide-in-from-bottom-10">
             ...
             <div className="grid grid-cols-2 gap-4">
                ...
                <button onClick={() => setState(s => ({ ...s, resultImage: null }))} className="col-span-2 py-4 text-gray-400 font-black text-[9px] uppercase tracking-widest active:scale-95">Примерить другое</button>
             </div>
          </div>
        ) : (
