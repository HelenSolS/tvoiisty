...
Критерии успеха и «как вспомнить» правила: **docs/SUCCESS-CRITERIA-WORKING-RULES.md** — не ломаем работающее, идём по плану, проверяем в dev, сложности обсуждаем, каноны чтим; чеклист на каждый шаг.
...
КАНОН ПРОВАЙДЕРОВ И МОДЕЛЕЙ — НЕ ТЕРЯТЬ
- Мы долго над этим работали. Канон не придумывать заново и не менять без явного решения.
- Примерка: первичен Fal (nano banana), вторичен KIE (nano banana). Ручной выбор моделей — только если в настройках открыта эта функция.
- Видео: основная KIE Grok, запасная KIE Kling v2-1 standard. Ручной выбор — в настройках.
- Полное описание: docs/CANON-PROVIDERS-AND-MODELS.md. При любых правках провайдеров/моделей — сверяться с каноном и не ломать порядок (Fal первичен, KIE вторичен; видео Grok → Kling).
